...
module DeliverableByVehiclesHelper
  ...
  def plannings_by_ids(customer, planning_ids, vehicle_id)
  ...
  def routes_total_infos(routes_quantities, routes)
    ...
    puts averages[:quantity_average] / routes.length
    {
      quantity_average: number_with_precision(averages[:quantity_average] / routes.length, precision: 2, delimiter: I18n.t('number.format.delimiter'), strip_insignificant_zeros: true),
      destinations_average: number_with_precision(averages[:destinations_average] / routes.length, precision: 2, delimiter: I18n.t('number.format.delimiter'), strip_insignificant_zeros: true),
      stops_average: number_with_precision(averages[:stops_average] / routes.length, precision: 2, delimiter: I18n.t('number.format.delimiter'), strip_insignificant_zeros: true),
      visits_duration_average: averages[:visits_time_average] / routes.length,
      drive_time_average: averages[:drive_time_average] / routes.length,
      route_duration_average: averages[:route_duration_average] / routes.length,
